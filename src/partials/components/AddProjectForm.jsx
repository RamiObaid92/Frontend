...
// Se till att sökvägen är rätt!
import { projectsApi, clientsApi, usersApi } from "../../api/auth";

const AddProjectForm = ({ onSuccess, onCancel }) => {
    // Behåll react-hook-form
    const { register, handleSubmit, reset } = useForm({
        defaultValues: {
            projectName: "", projectDescription: "", clientId: "",
            startDate: "", endDate: "", projectOwnerId: "",
            budget: "", projectImage: null,
        }
    });
    ...
    useEffect(() => {
        const loadDropdownData = async () => {
            try {
                ...
                console.error("AddProjectForm: Failed to load dropdown data:", dropdownError);
            }
        };
        loadDropdownData();
    }, []);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <div className="form-group">
                <label htmlFor="projectName">Projektnamn*</label><br />
                <input id="projectName" {...register("projectName", { required: true })} />
            </div>

            <div className="form-group">
                <label htmlFor="projectDescription">Beskrivning</label><br />
                <textarea id="projectDescription" {...register("projectDescription")} />
            </div>

            <div className="form-group">
                <label htmlFor="projectImage">Bild</label><br />
                <input type="file" id="projectImage" {...register("projectImage")} />
            </div>

            <div className="form-group">
                <label htmlFor="clientId">Klient*</label><br />
                <select id="clientId" {...register("clientId", { required: true })} defaultValue="">
            ...
            <div className="form-group">
                 <label htmlFor="projectOwnerId">Projektägare*</label><br />
                 <select id="projectOwnerId" {...register("projectOwnerId", { required: true })} defaultValue="">
            ...
            <div className="form-group">
                ...
                <input type="number" id="budget" step="0.01" {...register("budget")} placeholder="0.00" />
            </div>
